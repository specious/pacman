...
overlap st =
    let
        ...
        mapper g =
            ((sidesSwapped g st.pacman) || (pif g.pos == pacPos)) && (g.self /= Dead)
...
interact st =
    let
        ...
        killPoints =
            List.sum <| List.take numScared st.ghostPoints

        lifeLoss =
            not <| List.isEmpty scary
        ...
        st' =
            { st | timers = newTimers, soundControls = newSoundControls }
    in
        case ( scary, scared ) of
            ...
            _ ->
                { st'
                    | points = st.points + killPoints
                    , extraLives = livesLeft
                    , gameState =
                        if not lifeLoss then
                            Active
                        else
                            Dying
