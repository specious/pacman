...
btoa bd =
    Arr.fromList <| List.map (Arr.fromList << List.reverse) bd


atob : Array (Array Pac.Box) -> Pac.Board
atob bd =
    Arr.toList <| Arr.map (List.reverse << Arr.toList) bd
...
modifyRow cl_ix rw_ix bd =
    let
        rw =
            withDefault Array.empty (Arr.get rw_ix bd)

        updatedRow =
            Arr.set cl_ix Pac.Empty rw
    in
...
removeTreat ( x, y ) treat_fn bd =
    let
        ...
        ( cxl, cxr ) =
            ( clamp 0 (Pac.numCols - 1) <| floor x
            , clamp 0 (Pac.numCols - 1) <| ceiling x
            )

        ( cyu, cyd ) =
            ( clamp 0 (Pac.numRows - 1) <| floor y
            , clamp 0 (Pac.numRows - 1) <| ceiling y
            )

        rwu =
            withDefault Array.empty (Arr.get cyu bd')

        rwd =
            withDefault Array.empty (Arr.get cyd bd')

        bxul =
            withDefault Pac.Empty (Arr.get cxl rwu)

        bxur =
            withDefault Pac.Empty (Arr.get cxl rwd)

        bxdl =
            withDefault Pac.Empty (Arr.get cxr rwu)

        bxdr =
            withDefault Pac.Empty (Arr.get cxr rwd)
    in
...
updateBoard board_old pacman_old =
    let
        ( bPells, board_new1 ) =
            removeTreat pacman_old.pos isPellet board_old

        ( bPills, board_new2 ) =
            removeTreat pacman_old.pos isPill board_old
    in
        if bPells then
