...
displayBox b bSide =
    case b of
        Empty ->
            Clg.collage bSide bSide [ Mod.emptySpace (toFloat bSide) ]

        Pellet ->
            Clg.collage bSide bSide [ Mod.pellet ((toFloat bSide) / 6) ]

        Pill ->
            Clg.collage bSide bSide [ Mod.pill ((toFloat bSide) / 3) ]

        Wall ->
            Clg.collage bSide bSide [ Mod.wall (toFloat bSide) ]

        Gate ->
            Clg.collage bSide bSide [ Mod.gate (toFloat bSide) ]

        Fruit ->
            Clg.collage bSide bSide [ Mod.fruit ]
...
displayLives lives sz ravi =
    let
        life =
            Clg.collage sz sz [ Mod.pacman Mod.Right ((toFloat sz) / 2) ravi ]
    in
...
renderGhost g bSide ravi =
    ...
    in
        case g.self of
            Scared ->
                Mod.ghost "scared" w h ravi

            Dead ->
                Mod.ghost "dead" w h ravi

            Normal ->
                Mod.ghost g.name w h ravi
...
myButton msg str w h =
    let
        ...
        button_up str =
            button_general yellow str

        button_hover str =
            button_general lightYellow str

        button_down str =
            button_general darkYellow str
    in
        Clg.toForm
            <| Inp.customButton (Signal.message actionMailbox.address (ButtonAction msg))
                (button_up str)
                (button_hover str)
                (button_down str)
...
view ( w, h ) st =
    let
        ...
        rowBuilder bxs =
            El.flow El.left (List.map (\b -> displayBox b bSide) bxs)

        colBuilder rws =
            El.flow El.down ([ scoreLives ] ++ rws)
        ...
        inky_pos =
            Utl.itow wFromBxs hFromBxs st.inky.pos

        blinky_pos =
            Utl.itow wFromBxs hFromBxs st.blinky.pos

        clyde_pos =
            Utl.itow wFromBxs hFromBxs st.clyde.pos

        pac_pos =
            Utl.itow wFromBxs hFromBxs st.pacman.pos

        pac_dir =
            case st.pacman.dir of
                Left ->
                    Mod.Left
        ...
        pacSelf =
            case st.gameState of
                Dying ->
                    let
                        self =
                            Mod.animatePacman pac_dir (toFloat (bSide // 2)) ravi
                    in
                        unsafeFromJust <| List.head <| List.drop (dyingStates - st.dyingList) self

                _ ->
                    (Mod.pacman pac_dir <| toFloat <| bSide // 2) ravi

        ghosts =
            if st.gameState == Dying then
                Clg.toForm El.empty
            else
                Clg.group
                    [ Clg.move pinky_pos <| renderGhost st.pinky bSide ravi
...
arrows =
    dropMap (toUDLR { up = 38, down = 40, left = 37, right = 39 }) Key.keysDown
...
upstate a s =
    case ( a, s.gameState ) of
        ...
        ( KeyAction k, Active ) ->
            { s | pacman = Ctr.updateDir k s.pacman }

        ( TimeAction, Active ) ->
            let
                ( extra_pts, newBoard ) =
                    BCtr.updateBoard s.board s.pacman
                ...
                level_done =
                    new_pillsAte + new_pellsAte == totPills + totPells

                stopFlee =
                    s.timers.fleeTimer >= fleeTime
                ...
                s' =
                    { s
                        | pacman = Ctr.updatePacPos s.pacman newTimers.fleeTimerOn
                        , board = newBoard
        ...
        ( TimeAction, Over ) ->
            if s.timers.overTimer <= 0 then
                ...
                let
                    ...
                    newTimers =
                        { tmers | overTimer = tmers.overTimer - 0.025 }
                in
                    { s | timers = newTimers }
        ...
        ( ButtonAction Options, _ ) ->
            { s | gameState = OptMenu }

        ( ButtonAction Strt, _ ) ->
            { s | gameState = Start }

        ( ButtonAction (PicMode (Just b)), _ ) ->
            { s | raviMode = b }
...
deathPropertiesHandler props =
    if props.currentTime > props.duration then
        Just <| Aud.Pause
    else
        Nothing
...
deathHandleAudio st =
    if st.soundControls.dying then
        Aud.Play
    else
        Aud.Pause
...
introHandleAudio st =
    if st.soundControls.intro then
        Aud.Play
    else
        Aud.Pause
...
ghostHandleAudio st =
    if st.soundControls.eatGhost then
        Aud.Play
    else
        Aud.Pause
...
pellHandleAudio st =
    if st.soundControls.eatPellet then
        Aud.Play
    else
        Aud.Pause
...
main =
    view `Signal.map` Window.dimensions `andMap` currState
