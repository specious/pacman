...
isBarrier ( x, y ) careAboutGates =
    let
        isB =
            \b -> b == Pac.Wall || (b == Pac.Gate && careAboutGates)
        ...
        rwu =
            withDefault Array.empty (Array.get cyu wallLocs)

        rwd =
            withDefault Array.empty (Array.get cyd wallLocs)

        bxul =
            withDefault Pac.Empty (Array.get cxl rwu)

        bxur =
            withDefault Pac.Empty (Array.get cxl rwd)

        bxdl =
            withDefault Pac.Empty (Array.get cxr rwu)

        bxdr =
            withDefault Pac.Empty (Array.get cxr rwd)
    in
        if isB bxul || isB bxur || isB bxdl || isB bxdr then
            True
        else
            False
...
updatePos pos d delta =
    case d of
        Pac.Left ->
            pos_add pos ( -delta, 0.0 )

        Pac.Right ->
            pos_add pos ( delta, 0.0 )

        Pac.Up ->
            pos_add pos ( 0.0, -delta )
...
updatePacPos pacman_old ghosts_frightened =
    let
        ...
        delta =
            0.5
                * pacman_old.difMulti
                * (if ghosts_frightened then
                    pacman_old.spdMultis.scrd
                   else
                    pacman_old.spdMultis.norm
                  )

        new_pos =
            updatePos old_pos d delta
