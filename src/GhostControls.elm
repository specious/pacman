...
updateGhostPos g targ =
    let
        ...
        curDirOrBarrier d =
            case d of
                Left ->
                    g.dir /= Right && (not <| isBarrier ( gx - delta, gy ) <| notHome g)

                Right ->
                    g.dir /= Left && (not <| isBarrier ( gx + delta, gy ) <| notHome g)

                Up ->
                    g.dir /= Down && (not <| isBarrier ( gx, gy - delta ) <| notHome g)

                Down ->
                    g.dir /= Up && (not <| isBarrier ( gx, gy + delta ) <| notHome g)

        legal_dirs =
            Lst.filter curDirOrBarrier [ Left, Right, Up, Down ]
            ...
            case legal_dirs of
                ...
                _ ->
                    let
                        ...
                        fst_dir =
                            unsafeFromJust <| Lst.head dirs_and_dists
    ...
    let
        ...
        update g =
            let
                ...
                new_pos =
                    updatePos g.pos new_dir ghostPace
            in
                case g.self of
                    Dead ->
                        g
    ...
    in
        { st
            ...
            , modeChanges =
                case upd of
                    NoChange ->
                        unsafeFromJust (Lst.tail st.modeChanges)
...
makeFlee g =
    { g | self = Scared, mode = Flee }


updateGhosts : State -> Bool -> State
updateGhosts st atePill =
    if st.timers.fleeTimer < fleeTime && not atePill && (Lst.isEmpty st.modeChanges || st.timers.gameTimer < (unsafeFromJust <| Lst.head st.modeChanges)) then
        let
            b =
                st.blinky
            ...
            ( bt, bg ) =
                blinkyTarget b pac st.defaultMode st.pellsAte

            ( pt, pg ) =
                pinkyTarget p pac st.defaultMode st.pellsAte

            ( ct, cg ) =
                clydeTarget c pac st.defaultMode st.pellsAte

            ( it, ig ) =
                inkyTarget i b pac st.defaultMode st.pellsAte
        in
            { st
                | blinky = updateGhostPos bg bt
                , pinky = updateGhostPos pg pt
                , inky = updateGhostPos ig it
                , clyde = updateGhostPos cg ct
            }
    else if st.timers.fleeTimer >= fleeTime then
...
blinkyTarget g p dMode pells =
    case g.mode of
        ...
        Flee ->
            let
                gen =
                    R.pair (R.int 0 (numCols - 1)) (R.int 0 (numRows - 1))

                ( ( rx, ry ), new_seed ) =
                    R.generate gen g.seed
            in
                ( pfi ( rx, ry ), { g | seed = new_seed } )

        Inactive ->
            if (leaveHouse g pells) then
                ( ( 13, 11 ), { g | mode = House } )
            else
                ( initBlinky.pos, g )
    ...
    case i.mode of
        ...
        Flee ->
            let
                ...
                ( ( rx, ry ), new_seed ) =
                    R.generate gen i.seed
            in
                ( pfi ( rx, ry ), { i | seed = new_seed } )

        Inactive ->
            if (leaveHouse i pells) then
                ( ( 13, 11 ), { i | mode = House } )
            else
                ( initInky.pos, i )
