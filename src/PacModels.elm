...
pacman d r ravi =
    ...
    in
        if ravi then
            group
                [ toForm <| El.fittedImage s s "graphics/ravi.png"
                , filled black <| tri
                ]
        else
...
makeP c r t =
    group [ filled c <| circle r, t ]


makeR r t =
    group [ toForm <| El.fittedImage (2 * (floor r)) (2 * (floor r)) "graphics/ravi.png", t ]
...
animatePacman d r ravi =
    let
        ...
        ps =
            List.map radsToPts [ pi / 4, (5 * pi) / 16, (6 * pi) / 16, (7 * pi) / 16 ]

        makeTri ( x, y ) =
            group
                <| List.map (filled black)
                    [ polygon [ ( x, y ), ( r, r ), ( r, -r ), ( x, -y ) ]
                    , polygon [ ( 0, 0 ), ( x, y ), ( x, -y ) ]
                    ]
        ...
        other =
            (List.reverse ps) ++ (List.map radsToPts [ (3 * pi) / 16, (2 * pi) / 16, pi / 16, pi / 32, pi / 64 ])
    ...
    in
        case d of
            ...
            Left ->
                List.map (rotate pi) rightCase

            Up ->
                List.map (rotate (pi / 2)) rightCase

            Down ->
                List.map (rotate ((3 * pi) / 2)) rightCase
